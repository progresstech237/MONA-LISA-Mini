...
// NEW DOWNLOADER - FIX FOR conn.downloadMediaMessage is not a function
async function getBufferFromMsg(quotedOrMek) {
    let msg = quotedOrMek;
    // if it's the command message object with quoted
...
cmd({ pattern: "ttsx", alias: ["tts11"], react: "🗣️", desc: "TTS ElevenLabs - /api/ai/elevenlabs", category: "ai", use: ".ttsx Hello world", filename: __filename },
async (conn, mek, m, { from, q, reply }) => {
...
cmd({ pattern: "megagrokvideo", alias: ["megagrok3video"], react: "🤖", desc: "Grok 3 Video - /api/ai/grok-3-video", category: "progresstech ai", use: ".megagrokvideo prompt", filename: __filename },
async (conn, mek, m, { from, q, reply }) => {
    try { if (!q) return reply("*_Prompt needed_*");
...
// ==================== IMAGE FILTERS (REPLY IMAGE) - FIXED ====================
async function handleImageTool(conn, mek, m, from, q, reply, toolPath, toolName) {
    try {
        if (!m.quoted) return reply(`*_Reply to an image for ${toolName}:.${toolName} (reply image)_*`);
...
cmd({ pattern: "megaupscale", alias: ["image-upscaler","imgupscale"], react: "✨", desc: "Image Upscaler - /api/tools/Image-upscaler", category: "progresstech tools", use: ".megaupscale (reply image)", filename: __filename },
...
cmd({ pattern: "enhancevideo", alias: ["video-enhancer","wink-video-enhancer"], react: "🎥", desc: "Video Enhancer - /api/tools/Video-Enhancer & wink", category: "progresstech tools", use: ".enhancevideo (reply video)", filename: __filename },
async (conn, mek, m, { from, q, reply }) => {
    try {
    ...
    } catch (e) { reply(`*❌ Enhance Error was noticed:* ${e.message}`); }
});

//... rest of your stalk & downloader packs remain same