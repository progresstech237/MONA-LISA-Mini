...
┃
┃ *Usage:*
┃ ${prefix}zchatagent hello, how are you?
┃ ${prefix}zchatagent write me a poem about Mona Lisa
┃ ${prefix}zchatagent explain quantum computing
┃ ${prefix}zchatagent clear - new session + fresh creds
┗━━━━━━━━━━━━━━┛
`;

        const buttons = [
            { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "💬 Hello", id: `${prefix}zchatagent hello, introduce yourself` }) },
            { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "📝 Write Poem", id: `${prefix}zchatagent write a romantic poem about Mona Lisa afro girl` }) },
            { name: "quick_reply", buttonParamsJson: JSON.stringify({ display_text: "🔄 New Session", id: `${prefix}zchatagent clear` }) },
    ...
    if (answer.length > 4000) {
        ...
        await conn.sendMessage(from, {
            text: `*🤖 ZChat:*\n\n${answer}\n\n_${BRAND}_\nSession: ${sessionId.slice(0,15)}... | ${prefix}zchatagent clear for fresh creds`,
            contextInfo: ctx
        }, { quoted: mek });
    }
    ...
    reply(`*❌ ZChat Failed*\n${e.response?.data?.message || e.message}\n\nTry: ${'.zchatagent clear'} then ask again (fresh creds)`);
  }
});